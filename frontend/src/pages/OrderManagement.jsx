...
export default function OrderManagement() {
  ...
  const getStatusConfig = (status) => {
     switch(status?.toUpperCase()) {
        ...
        case 'OUT_FOR_DELIVERY': return { icon: Truck, color: 'text-orange-500 bg-orange-500/10', label: 'On Way' };
  ...
  const orderFlow = ['PENDING', 'CONFIRMED', 'PREPARING', 'OUT_FOR_DELIVERY', 'DELIVERED'];
    ...
    <AdminLayout>
      <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between mb-10">
        <div className="space-y-1">
          ...
          <h1 className="font-display text-4xl font-black tracking-tight">Fulfillment Hub</h1>
        </div>
        <div className="flex items-center gap-4">
            <div className="text-right hidden sm:block pr-4 border-r border-black/5 dark:border-white/5">
                ...
                <p className="text-2xl font-black">{orders.filter(o => !['DELIVERED', 'CANCELLED'].includes(o.status)).length}</p>
            </div>
            <Button variant="secondary" icon={Clock} onClick={loadOrders}>Refresh</Button>
        </div>
      </div>

      <div className="mb-8 flex flex-wrap gap-2 p-1 bg-gray-100 dark:bg-gray-800 rounded-2xl w-fit">
         {tabs.map(tab => (
           <button
             key={tab}
      ...
      <Card className="overflow-hidden border-black/5 dark:border-white/5 shadow-2xl">
        <div className="p-6 border-b border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 text-xs font-black text-gray-500 uppercase tracking-widest px-4 whitespace-nowrap">
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
              {filtered.map((order) => {
                ...
                const nextStatusIndex = orderFlow.indexOf(order.status) + 1;
                const nextStatus = nextStatusIndex < orderFlow.length ? orderFlow[nextStatusIndex] : null;
        ...
        {selectedOrder && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/70 backdrop-blur-md">
            <motion.div 
              ...
              className="w-full max-w-3xl bg-white rounded-[32px] p-10 shadow-huge dark:bg-gray-800 relative max-h-[90vh] overflow-y-auto"
            >
              <div className="mb-10 flex items-center justify-between">
                <div className="space-y-2">
                    <div className="flex items-center gap-3">
                        <h3 className="text-3xl font-black tracking-tight">Order Details</h3>
                    ...
                    <p className="text-gray-500 text-sm font-bold flex items-center gap-2">
                        <Clock size={14} /> Received on {new Date(selectedOrder.createdTs).toLocaleString()}
                    </p>
                </div>
                <button onClick={() => setSelectedOrder(null)} className="h-12 w-12 rounded-full hover:bg-gray-100 flex items-center justify-center dark:hover:bg-gray-700 transition-colors">
                    <XCircle size={28} />
                </button>
              </div>
              ...
              <div className="grid gap-10 lg:grid-cols-2">
                 ...
                 <div className="space-y-8">
                    ...
                    <div className="pt-8 border-t-2 border-black/5 dark:border-white/5 space-y-8">
                       <div className="flex justify-between items-center">
                          <div>
                             <p className="text-sm font-black text-gray-400 uppercase tracking-widest">Grand Total</p>
                             <p className="text-4xl font-black text-emerald-600 tracking-tight">₹{selectedOrder.totalAmount.toLocaleString()}</p>
                          </div>
                          <Badge variant="success" className="px-4 py-2 bg-emerald-500/10 text-emerald-600 font-black border-0 rounded-xl">PAID • ONLINE</Badge>
                       </div>
